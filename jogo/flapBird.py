...
IMAGEM_BACKGROND = pygame.transform.scale2x(pygame.image.load(os.path.join('imgs','bg.png')))
IMAGENS_PASSARO = [
...
class Passaro:
    ...
    def mover(self):
        ...
        if(deslocamento < 0 or self.y < (self.altura + 50)):
            ...
            if (self.angulo > -90):
                 self.angulo -= self.VELOCIDADE_ROTAÇÃO
    ...
    def desenhar(self,tela):
        ...
        #se o passaro estiver caindo eu nao vou bater asa
        if (self.angulo <= -80):
            self.imagem = self.IMGS[1]
            self.contagem_imagem = self.TEMPO_ANIMAÇÃO*2
...
def desenhar_tela(tela,passaros,canos,chão,pontos):
    tela.blit(IMAGEM_BACKGROND,(0,0))
...
def main():
    ...
    while rodando:
        ...
        for i , passaro in enumerate(passaros):
            if (passaro.y + passaro.imagem.get_height()) > chão.y or passaro.y < 0:
                passaros.pop(i)
    
        desenhar_tela(tela , passaros , canos , chão , pontos)
